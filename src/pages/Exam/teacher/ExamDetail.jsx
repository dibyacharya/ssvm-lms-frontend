...
const statusColors = {
  ...
  live: 'bg-primary-50 text-primary-600',
  completed: 'bg-emerald-900/30 text-emerald-600',
...
const submissionStatusColors = {
  ...
  graded: 'bg-emerald-900/30 text-emerald-600',
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {exam.status === 'draft' && (
              <>
                <button onClick={handlePublish} className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-primary-600 to-primary-500 text-gray-900 rounded-xl text-xs font-semibold hover:from-primary-700 hover:to-primary-600">
                  <FaPlay /> Publish
                </button>
                <button onClick={() => navigate(`/teacher/exam/create/${exam.course}?edit=${examId}`)} className="flex items-center gap-1.5 px-3 py-2 bg-white/70 border border-gray-200 rounded-xl text-xs font-semibold text-gray-600 hover:bg-gray-50">
                  <FaEdit /> Edit
                </button>
            ...
            {['scheduled', 'live'].includes(exam.status) && (
              <button onClick={handleCancel} className="flex items-center gap-1.5 px-3 py-2 bg-red-50 text-red-600 rounded-xl text-xs font-semibold hover:bg-red-900/50">
                <FaBan /> Cancel
              </button>
            )}
          </div>
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 bg-white/70 backdrop-blur-xl rounded-xl border border-gray-200 p-1">
          {tabs.map(t => (
            <button
              key={t.id}
              onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold transition-colors ${
                activeTab === t.id ? 'bg-primary-600 text-gray-900' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
              }`}
            >
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.15 }}>
            ...
            {activeTab === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Schedule Card */}
                <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200 p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><FaClock className="text-primary-600" /> Schedule</h3>
                ...
                {/* Settings Card */}
                <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200 p-5">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><FaClipboardList className="text-primary-600" /> Settings</h3>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span className="text-gray-500">Shuffle Questions</span><span className={exam.shuffleQuestions ? 'text-primary-600' : 'text-gray-400'}>{exam.shuffleQuestions ? 'Yes' : 'No'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-500">Shuffle Options</span><span className={exam.shuffleOptions ? 'text-primary-600' : 'text-gray-400'}>{exam.shuffleOptions ? 'Yes' : 'No'}</span></div>
                    <div className="flex justify-between"><span className="text-gray-500">Show Results</span><span className={exam.showResultsToStudent ? 'text-primary-600' : 'text-gray-400'}>{exam.showResultsToStudent ? 'Yes' : 'No'}</span></div>
                ...
                {/* Proctoring Card */}
                <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200 p-5 md:col-span-2">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><FaShieldAlt className="text-primary-600" /> Proctoring</h3>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        <div key={item.label} className="flex items-center gap-2 text-xs">
                          {item.val ? <FaCheck className="text-primary-600" /> : <FaTimes className="text-gray-500" />}
              ...
              <div className="space-y-2">
                {exam.questions?.length === 0 ? (
                  ...
                  exam.questions?.map((q, i) => (
                    <div key={q._id || i} className="bg-white/70 backdrop-blur-xl rounded-xl border border-gray-200 p-4">
                      <div className="flex items-start gap-3">
                        <span className="flex-shrink-0 w-7 h-7 rounded-lg bg-primary-50 text-primary-600 flex items-center justify-center text-xs font-bold">{i + 1}</span>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-gray-900">{q.question}</p>
                          {q.type === 'mcq' && q.options?.length > 0 && (
                            <div className="mt-2 space-y-1">
                              {q.options.map((opt, j) => (
                                <div key={j} className={`text-xs px-2 py-1 rounded ${opt === q.correctAnswer ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-500'}`}>
                          ...
                          {q.type === 'true_false' && (
                            <p className="text-xs mt-1 text-primary-600 font-medium">Answer: {q.correctAnswer}</p>
                          )}
                          {q.type === 'fill_in_blank' && q.correctAnswer && (
                            <p className="text-xs mt-1 text-primary-600 font-medium">Answer: {q.correctAnswer}</p>
                          )}
                          <div className="flex items-center gap-2 mt-2">
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-primary-50 text-primary-600">{q.type?.replace('_', ' ')}</span>
                ...
                {exam.questionSelection === 'random_from_bank' && exam.randomConfig && (
                  <div className="bg-primary-900/20 rounded-xl border border-primary-500/20 p-4">
                    <h4 className="text-sm font-semibold text-primary-200 mb-2">Random Selection Config</h4>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div><span className="text-primary-600">Total:</span> <span className="font-medium text-primary-200">{exam.randomConfig.totalQuestions}</span></div>
                      {Object.entries(exam.randomConfig.perDifficulty || {}).map(([k, v]) => (
                        <div key={k}><span className="text-primary-600 capitalize">{k}:</span> <span className="font-medium text-primary-200">{v}</span></div>
                      ))}
                    </div>
                  </div>
                )}
            ...
            {activeTab === 'submissions' && (
              <div className="space-y-3">
                    ...
                    {exam.status === 'completed' && (
                      <>
                        <button
                          ...
                          disabled={exporting}
                          className="flex items-center gap-1.5 px-3 py-2 bg-white/70 border border-gray-200 rounded-xl text-xs font-semibold text-gray-600 hover:bg-gray-50"
                        >
                ...
                {submissions.length === 0 ? (
                  ...
                  <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200 overflow-hidden">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {submissions.map(sub => (
                          <tr key={sub._id} className="border-b border-white/[0.05] hover:bg-white/[0.02]">
                            ...
                            <td className="px-4 py-3 text-center">
                              <span className={`text-xs font-bold ${
                                (sub.integrityScore ?? 100) >= 80 ? 'text-primary-600' :
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                      {[
                        ...
                        <div key={s.label} className="bg-white/70 backdrop-blur-xl rounded-xl border border-gray-200 p-4 text-center">
                            ...
                            <div key={i} className="flex items-center gap-3 py-2 border-b border-white/[0.05] last:border-0">
                              ...
                              <div className="flex items-center gap-4 text-xs">
                                <div className="text-center">
                                  <p className={`font-bold ${qs.difficultyIndex >= 0.7 ? 'text-primary-600' : qs.difficultyIndex >= 0.4 ? 'text-amber-600' : 'text-red-600'}`}>
                                    {(qs.difficultyIndex * 100).toFixed(0)}%
                                  </p>
                                  <p className="text-[10px] text-gray-400">Correct</p>
                                </div>
                                <div className="text-center">
                                  <p className={`font-bold ${qs.discriminationIndex >= 0.3 ? 'text-primary-600' : qs.discriminationIndex >= 0.1 ? 'text-amber-600' : 'text-red-600'}`}>
                    ...
                    {analytics.proctoringSummary && (
                      <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-gray-200 p-5">
                        <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2"><FaShieldAlt className="text-primary-600" /> Proctoring Summary</h3>
