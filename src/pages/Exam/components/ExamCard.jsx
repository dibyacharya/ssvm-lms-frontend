...
const STATUS_CONFIG = {
  draft: { label: 'Draft', icon: FaClock, color: 'bg-gray-100 text-gray-600', dot: 'bg-gray-400' },
  scheduled: { label: 'Scheduled', icon: FaClock, color: 'bg-blue-100 text-blue-700', dot: 'bg-blue-500' },
  live: { label: 'Live Now', icon: FaPlay, color: 'bg-emerald-100 text-emerald-700', dot: 'bg-emerald-500 animate-pulse' },
  completed: { label: 'Completed', icon: FaCheckCircle, color: 'bg-indigo-100 text-indigo-700', dot: 'bg-indigo-500' },
  cancelled: { label: 'Cancelled', icon: FaBan, color: 'bg-red-100 text-red-700', dot: 'bg-red-500' },
};
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        <div>
          ...
          <p className="text-gray-700 font-medium">{formatDate(exam.scheduledStartTime)}</p>
        </div>
        <div>
          <span className="text-gray-400">End</span>
          <p className="text-gray-700 font-medium">{formatDate(exam.scheduledEndTime)}</p>
        </div>
      </div>
